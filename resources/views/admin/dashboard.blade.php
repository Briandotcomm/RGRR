...
<div class="stats-grid">
  ...
  <div class="stat-card" style="background:var(--card);border:1px solid rgba(37,99,235,0.22);border-top:3px solid #2563eb;border-radius:16px;padding:28px 24px;">
    ...
    <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:2.2rem;color:var(--text);line-height:1;">{{ $memberCount }}</div>
  ...
  <div class="content-card">
    ...
    <div>
      @forelse($recentActivity as $activity)
        <div style="display:flex;gap:12px;padding:14px 22px;border-bottom:1px solid rgba(37,99,235,0.06);">
          <div style="width:8px;height:8px;border-radius:50%;background:{{ $activity['color'] }};margin-top:6px;flex-shrink:0;"></div>
          <div>
            <div style="font-size:0.82rem;line-height:1.6;color:var(--muted);">{!! $activity['text'] !!}</div>
            <div style="font-size:0.72rem;color:var(--muted);margin-top:2px;">{{ $activity['time'] }}</div>
          </div>
        </div>
      @empty
        <div style="padding:24px;text-align:center;color:var(--muted);font-size:0.85rem;">No recent activity.</div>
      @endforelse
    </div>
  </div>
